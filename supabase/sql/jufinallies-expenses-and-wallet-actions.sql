-- Jufinallies expenses, finance permissions, refunds and wallet transfers
-- Run once in the Jufinallies Supabase SQL Editor before deploying the updated index.html.
...
create or replace function public.jufinallies_has_permission(
  p_permission text,
...
begin
  ...
  where is_active = true
    and business_id = public.current_jufinallies_business_id()
    and (
...
revoke all on function public.jufinallies_has_permission(text,text[]) from public;
grant execute on function public.jufinallies_has_permission(text,text[]) to authenticated, service_role;

create table if not exists public.jufinallies_expenses (
...
create index if not exists jufinallies_expenses_business_date_idx
  on public.jufinallies_expenses (business_id, expense_date desc, created_at desc);
create index if not exists jufinallies_expenses_status_idx
  on public.jufinallies_expenses (business_id, status, expense_date desc);

drop trigger if exists jufinallies_expenses_updated_at on public.jufinallies_expenses;
create trigger jufinallies_expenses_updated_at before update on public.jufinallies_expenses
for each row execute function public.set_updated_at();

alter table public.jufinallies_expenses enable row level security;
drop policy if exists jufinallies_expenses_select on public.jufinallies_expenses;
create policy jufinallies_expenses_select on public.jufinallies_expenses for select to authenticated
using (
  business_id = public.current_jufinallies_business_id()
  and (
    public.jufinallies_has_permission('view_expenses',array['admin']::text[])
    or public.jufinallies_has_permission('view_accounting',array['admin']::text[])
  )
);

revoke insert, update, delete on public.jufinallies_expenses from anon, authenticated;
grant select on public.jufinallies_expenses to authenticated;
grant all on public.jufinallies_expenses to service_role;

create or replace function public.jufinallies_create_expense(
  p_expense_date date,
  p_category text,
  p_description text,
  p_amount numeric,
...
as $$
declare
  v_business_id text := public.current_jufinallies_business_id();
  v_staff_id uuid;
  v_expense public.jufinallies_expenses%rowtype;
  v_expense_no text;
begin
  if v_business_id is null then raise exception 'No active Jufinallies staff account was found.'; end if;
  if not public.jufinallies_has_permission('create_expenses',array['admin','branch_manager']::text[]) then
  ...
  insert into public.jufinallies_expenses (
  ...
  values (v_business_id,v_staff_id,'expense_initiated','jufinallies_expenses',v_expense.id::text,to_jsonb(v_expense));
  return to_jsonb(v_expense);
end;
$$;

create or replace function public.jufinallies_decide_expense(
  p_expense_id uuid,
  p_decision text,
...
declare
  ...
  v_cash_account text;
begin
  if not public.jufinallies_has_permission('approve_expenses',array['admin']::text[]) then
  ...
  select * into v_expense from public.jufinallies_expenses
  ...
  update public.jufinallies_expenses set
  ...
  values (v_business_id,v_staff_id,'expense_'||v_decision,'jufinallies_expenses',v_expense.id::text,
...
create or replace function public.jufinallies_client_charge_balance(p_client_id uuid)
returns numeric
language sql
stable
security definer
...
as $$
  ...
  where business_id = public.current_jufinallies_business_id()
    and client_id = p_client_id;
$$;

create or replace function public.jufinallies_refund_client_charge(
  p_client_id uuid,
  p_amount numeric,
...
begin
  if not public.jufinallies_has_permission('manage_charge_adjustments',array['admin']::text[]) then
  ...
  select public.jufinallies_client_charge_balance(p_client_id) into v_balance;
...
create or replace function public.jufinallies_transfer_client_charge(
...
begin
  ...
  select public.jufinallies_client_charge_balance(p_from_client_id) into v_balance;
  ...
  select public.jufinallies_client_charge_balance(p_from_client_id) into v_balance;
  select public.jufinallies_client_charge_balance(p_to_client_id) into v_to_balance;
  return jsonb_build_object('ok',true,'from_balance',v_balance,'to_balance',v_to_balance,'reference',v_ref);
end;
$$;

create or replace function public.jufinallies_transfer_charge_to_loan(
  p_from_client_id uuid,
  p_loan_id uuid,
  p_amount numeric,
  p_transaction_date date,
...
revoke all on function public.jufinallies_create_expense(date,text,text,numeric,text,text) from public;
revoke all on function public.jufinallies_decide_expense(uuid,text,text) from public;
revoke all on function public.jufinallies_refund_client_charge(uuid,numeric,date,text,text,text) from public;
revoke all on function public.jufinallies_transfer_client_charge(uuid,uuid,numeric,date,text,text) from public;
revoke all on function public.jufinallies_transfer_charge_to_loan(uuid,uuid,numeric,date,text,text) from public;
grant execute on function public.jufinallies_create_expense(date,text,text,numeric,text,text) to authenticated,service_role;
grant execute on function public.jufinallies_decide_expense(uuid,text,text) to authenticated,service_role;
grant execute on function public.jufinallies_refund_client_charge(uuid,numeric,date,text,text,text) to authenticated,service_role;
grant execute on function public.jufinallies_transfer_client_charge(uuid,uuid,numeric,date,text,text) to authenticated,service_role;
grant execute on function public.jufinallies_transfer_charge_to_loan(uuid,uuid,numeric,date,text,text) to authenticated,service_role;

select 'Jufinallies expenses and wallet actions ready' as result;
