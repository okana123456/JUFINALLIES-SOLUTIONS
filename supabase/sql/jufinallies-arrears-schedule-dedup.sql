-- Jufinallies arrears schedule safeguard and repair.
-- Run once in the Jufinallies Supabase SQL Editor.
-- Legitimate instalments with different instalment numbers are not changed.

begin;

create table if not exists public.jufinallies_duplicate_schedule_merge_20260731 (
...
alter table public.jufinallies_duplicate_schedule_merge_20260731 enable row level security;
revoke all on table public.jufinallies_duplicate_schedule_merge_20260731 from anon, authenticated;
grant all on table public.jufinallies_duplicate_schedule_merge_20260731 to service_role;

delete from public.jufinallies_duplicate_schedule_merge_20260731;

insert into public.jufinallies_duplicate_schedule_merge_20260731
select
  loan_id,
  installment_no,
...
create table if not exists public.jufinallies_schedule_duplicate_backup_20260731
as select * from public.loan_schedules with no data;

alter table public.jufinallies_schedule_duplicate_backup_20260731 enable row level security;
revoke all on table public.jufinallies_schedule_duplicate_backup_20260731 from anon, authenticated;
grant all on table public.jufinallies_schedule_duplicate_backup_20260731 to service_role;

insert into public.jufinallies_schedule_duplicate_backup_20260731
select schedule.*
from public.loan_schedules schedule
join public.jufinallies_duplicate_schedule_merge_20260731 duplicate
  on duplicate.loan_id = schedule.loan_id
 and duplicate.installment_no = schedule.installment_no
where not exists (
  select 1
  from public.jufinallies_schedule_duplicate_backup_20260731 backup
...
set due_date = duplicate.due_date,
    ...
    updated_at = now()
from public.jufinallies_duplicate_schedule_merge_20260731 duplicate
where schedule.id = duplicate.keep_id;

delete from public.loan_schedules schedule
using public.jufinallies_duplicate_schedule_merge_20260731 duplicate
...
from public.jufinallies_duplicate_schedule_merge_20260731;
