-- Jufinallies complete database setup
-- Run this entire file once in the NEW Jufinallies Supabase SQL Editor.
...
create table if not exists public.loan_settings (
  ...
  company_name text not null default 'Jufinallies',
  company_phone text,
  company_email text,
...
create table if not exists public.jufinallies_daraja_credentials (
...
alter table public.jufinallies_daraja_credentials enable row level security;
revoke all on table public.jufinallies_daraja_credentials from anon, authenticated;
grant all on table public.jufinallies_daraja_credentials to service_role;
...
create table if not exists public.loans (
  ...
  loan_form_fee numeric(14,2) not null default 0,
  credit_report_fee numeric(14,2) not null default 0,
...
create or replace function public.current_jufinallies_business_id()
returns text
language sql
stable
security definer
...
revoke all on function public.current_jufinallies_business_id() from public;
grant execute on function public.current_jufinallies_business_id() to anon, authenticated, service_role;
...
drop policy if exists jufinallies_staff_select on public.loan_staff;
create policy jufinallies_staff_select on public.loan_staff for select to authenticated
using (auth_user_id = auth.uid() or business_id = public.current_jufinallies_business_id());
drop policy if exists jufinallies_staff_insert on public.loan_staff;
create policy jufinallies_staff_insert on public.loan_staff for insert to authenticated
with check (business_id = public.current_jufinallies_business_id());
drop policy if exists jufinallies_staff_update on public.loan_staff;
create policy jufinallies_staff_update on public.loan_staff for update to authenticated
using (auth_user_id = auth.uid() or business_id = public.current_jufinallies_business_id())
with check (business_id = public.current_jufinallies_business_id());
drop policy if exists jufinallies_staff_delete on public.loan_staff;
create policy jufinallies_staff_delete on public.loan_staff for delete to authenticated
...
begin
  foreach table_name in array array[
    ...
    execute format('drop policy if exists jufinallies_business_select on public.%I', table_name);
    execute format('drop policy if exists jufinallies_business_insert on public.%I', table_name);
    execute format('drop policy if exists jufinallies_business_update on public.%I', table_name);
    execute format('drop policy if exists jufinallies_business_delete on public.%I', table_name);
    execute format('create policy jufinallies_business_select on public.%I for select to authenticated using (business_id = public.current_jufinallies_business_id())', table_name);
    execute format('create policy jufinallies_business_insert on public.%I for insert to authenticated with check (business_id = public.current_jufinallies_business_id())', table_name);
    execute format('create policy jufinallies_business_update on public.%I for update to authenticated using (business_id = public.current_jufinallies_business_id()) with check (business_id = public.current_jufinallies_business_id())', table_name);
    execute format('create policy jufinallies_business_delete on public.%I for delete to authenticated using (business_id = public.current_jufinallies_business_id())', table_name);
  end loop;
end $$;
...
drop policy if exists jufinallies_photo_read on storage.objects;
create policy jufinallies_photo_read on storage.objects for select
using (bucket_id in ('jufinallies-client-photos','jufinallies-client-assets'));
drop policy if exists jufinallies_photo_insert on storage.objects;
create policy jufinallies_photo_insert on storage.objects for insert to authenticated
with check (bucket_id in ('jufinallies-client-photos','jufinallies-client-assets'));
drop policy if exists jufinallies_photo_update on storage.objects;
create policy jufinallies_photo_update on storage.objects for update to authenticated
using (bucket_id in ('jufinallies-client-photos','jufinallies-client-assets'))
with check (bucket_id in ('jufinallies-client-photos','jufinallies-client-assets'));
drop policy if exists jufinallies_photo_delete on storage.objects;
create policy jufinallies_photo_delete on storage.objects for delete to authenticated
using (bucket_id in ('jufinallies-client-photos','jufinallies-client-assets'));
