-- Jufinallies client charges wallet
-- Run this file once in the Jufinallies Supabase SQL Editor before deploying index.html.
...
create table if not exists public.client_charge_transactions (
  ...
  charge_type text not null default 'other' check (charge_type in ('processing','loan_form','credit_report','excess','other')),
...
drop policy if exists jufinallies_business_select on public.client_charge_transactions;
create policy jufinallies_business_select on public.client_charge_transactions for select to authenticated
using (business_id = public.current_jufinallies_business_id());

drop policy if exists jufinallies_business_insert on public.client_charge_transactions;
create policy jufinallies_business_insert on public.client_charge_transactions for insert to authenticated
with check (business_id = public.current_jufinallies_business_id());

drop policy if exists jufinallies_business_update on public.client_charge_transactions;
create policy jufinallies_business_update on public.client_charge_transactions for update to authenticated
using (business_id = public.current_jufinallies_business_id())
with check (business_id = public.current_jufinallies_business_id());

drop policy if exists jufinallies_business_delete on public.client_charge_transactions;
create policy jufinallies_business_delete on public.client_charge_transactions for delete to authenticated
using (business_id = public.current_jufinallies_business_id());
...
create or replace function public.jufinallies_deposit_client_charge(
  p_client_id uuid,
  p_amount numeric,
...
begin
  if v_business_id is null then
    raise exception 'No active Jufinallies staff account was found.';
  end if;
  ...
  insert into public.client_charge_transactions (
    ...
    case when p_charge_type in ('processing','loan_form','credit_report','excess','other') then p_charge_type else 'other' end,
...
create or replace function public.jufinallies_consume_client_charges(
  p_client_id uuid,
  p_loan_id uuid,
  p_processing_amount numeric,
  p_loan_form_amount numeric,
  p_credit_report_amount numeric,
  p_transaction_date date,
  p_reference text default null
)
returns jsonb
...
declare
  ...
  v_staff_id uuid;
  v_balance numeric(14,2);
  v_required numeric(14,2) := round(coalesce(p_processing_amount,0) + coalesce(p_loan_form_amount,0) + coalesce(p_credit_report_amount,0),2);
begin
  if v_business_id is null then
    raise exception 'No active Jufinallies staff account was found.';
  end if;
  if coalesce(p_processing_amount,0) < 0 or coalesce(p_loan_form_amount,0) < 0 or coalesce(p_credit_report_amount,0) < 0 then
  ...
  if coalesce(p_loan_form_amount,0) > 0 then
    insert into public.client_charge_transactions (
      business_id,client_id,loan_id,transaction_type,charge_type,amount,
      transaction_date,reference,description,source_key,created_by
    ) values (
      v_business_id,p_client_id,p_loan_id,'fee_debit','loan_form',round(p_loan_form_amount,2),
      coalesce(p_transaction_date,current_date),nullif(trim(p_reference),''),
      'Loan form fee used during disbursement','disbursement:'||p_loan_id::text||':loan-form',v_staff_id
    ) on conflict (business_id, source_key) where source_key is not null do nothing;
  end if;

  if coalesce(p_credit_report_amount,0) > 0 then
    insert into public.client_charge_transactions (
      business_id,client_id,loan_id,transaction_type,charge_type,amount,
      transaction_date,reference,description,source_key,created_by
    ) values (
      v_business_id,p_client_id,p_loan_id,'fee_debit','credit_report',round(p_credit_report_amount,2),
      coalesce(p_transaction_date,current_date),nullif(trim(p_reference),''),
      'Credit report fee used during disbursement','disbursement:'||p_loan_id::text||':credit-report',v_staff_id
    ) on conflict (business_id, source_key) where source_key is not null do nothing;
  end if;
...
revoke all on function public.jufinallies_client_charge_balance(uuid) from public;
revoke all on function public.jufinallies_deposit_client_charge(uuid,numeric,text,text,date,text,text,text,text) from public;
revoke all on function public.jufinallies_consume_client_charges(uuid,uuid,numeric,numeric,numeric,date,text) from public;
grant execute on function public.jufinallies_client_charge_balance(uuid) to authenticated, service_role;
grant execute on function public.jufinallies_deposit_client_charge(uuid,numeric,text,text,date,text,text,text,text) to authenticated, service_role;
grant execute on function public.jufinallies_consume_client_charges(uuid,uuid,numeric,numeric,numeric,date,text) to authenticated, service_role;
...
select
  ...
  case when lower(coalesce(j.credit,'')) like '%registration%' then 'loan_form' else 'processing' end,
...
with historic_fees as (
  select distinct on (j.id)
    ...
    case when lower(coalesce(j.credit,'')) like '%registration%' then 'loan_form' else 'processing' end charge_type
  from public.journal_entries j
