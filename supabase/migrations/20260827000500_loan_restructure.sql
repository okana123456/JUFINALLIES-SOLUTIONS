-- Jufinallies loan restructuring
-- Run this file once in Supabase SQL Editor before using the Restructure action.

create or replace function public.jufinallies_restructure_loan(
  p_loan_id uuid,
...
declare
  ...
  v_remaining_interest numeric(14,2) := 0;
...
begin
  if v_business_id is null then
    raise exception 'Your Jufinallies session is not linked to an active business. Please sign in again.';
  end if;
  ...
  -- Jufinallies products use an approved fixed-price schedule. Restructuring
  -- redistributes the existing balance and never creates new interest.
  v_new_outstanding := round(v_loan.outstanding_balance, 2);
  v_remaining_interest := greatest(
    0,
    round(v_new_outstanding - v_remaining_principal - v_remaining_penalty, 2)
  );
  v_new_interest := v_remaining_interest;
  ...
  insert into public.loan_audit_log (
    ...
    jsonb_build_object(
      ...
      'remaining_interest', v_new_interest,
      'interest_rate_per_period', v_loan.interest_rate,
...
revoke all on function public.jufinallies_restructure_loan(uuid, integer, date, integer, text) from public, anon;
grant execute on function public.jufinallies_restructure_loan(uuid, integer, date, integer, text) to authenticated;

comment on function public.jufinallies_restructure_loan(uuid, integer, date, integer, text)
is 'Archives the unpaid schedule and redistributes the existing outstanding balance without adding new interest.';
