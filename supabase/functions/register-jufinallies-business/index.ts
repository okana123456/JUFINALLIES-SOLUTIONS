...
serve(async (req) => {
  ...
  try {
    ...
    const expectedKey = String(Deno.env.get("JUFINALLIES_REGISTRATION_KEY") || "").trim();
    ...
    if (!await secureEqual(suppliedKey, expectedKey)) {
      return json({ ok: false, message: "Invalid business registration key. Contact the Jufinallies platform owner." }, 403);
    }

    const supabaseUrl = Deno.env.get("JUFINALLIES_PROJECT_URL") || "";
    const anonKey = Deno.env.get("JUFINALLIES_ANON_KEY") || "";
    const serviceRoleKey = Deno.env.get("JUFINALLIES_SERVICE_ROLE_KEY") || "";
    ...
    await admin.from("loan_products").insert([
      ["CHEMSHA", 3000, 5000, 4, 6],
      ["INUKA", 6000, 10000, 4, 8],
      ["JIFUNGIE", 11000, 15000, 4, 8],
      ["JITEGEMEE", 16000, 20000, 4, 8],
      ["NAWIRI", 21000, 30000, 6, 6],
    ].map(([name, min, max, minTerm, maxTerm]) => ({
      business_id: businessId,
      name,
      description: `${name} fixed pricing schedule | weekly repayments`,
      min_amount: min,
      max_amount: max,
      interest_rate: 0,
      interest_period: "fixed_schedule",
      interest_type: "flat",
      min_term_weeks: minTerm,
      max_term_weeks: maxTerm,
