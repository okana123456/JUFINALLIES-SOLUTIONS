...
serve(async (req) => {
  ...
  try {
    ...
    const payload = {
      ...
      AccountReference: "JUFINALLIES",
      TransactionDesc: "Jufinallies subscription diagnostic",
    };
    ...
    return json({ ok: true, message: "STK accepted. Jufinallies service Daraja setup works.", report });
  } catch (error) {
