...
serve(async (req) => {
  console.log("Jufinallies callback request", { method: req.method, url: req.url });
  ...
  try {
    ...
    console.log("Jufinallies callback payload", {
      transId,
    ...
    if (!transId) {
      console.log("Jufinallies callback ignored: missing TransID");
      return accepted();
    }
    ...
    if (!shortcode || !amount || amount <= 0) {
      console.log("Jufinallies callback ignored: missing shortcode or amount", { transId, shortcode, amount });
      return accepted();
    }

    const supabase = createClient(
      Deno.env.get("JUFINALLIES_PROJECT_URL") || "",
      Deno.env.get("JUFINALLIES_SERVICE_ROLE_KEY") || "",
    ...
    if (existingCallback) {
      ...
      console.log("Jufinallies duplicate callback ignored", { transId });
      return accepted();
    }
    ...
    console.log("Jufinallies callback business lookup", { transId, shortcode, businessId, autoConfirm: settings?.mpesa_auto_confirm });
    ...
    if (queueError) {
      if (String(queueError.message || "").toLowerCase().includes("duplicate")) {
        console.log("Jufinallies duplicate callback insert ignored", { transId });
        return accepted();
      }
      throw queueError;
    }
      ...
      if (queue?.id) {
        await supabase
          ...
          .update({
            ...
            processing_message: `No Jufinallies business matched shortcode ${shortcode}`,
          })
          .eq("id", queue.id);
      }
      console.log("Jufinallies callback stored without business match", { transId, shortcode });
      return accepted();
    }

    const client = await findClient(supabase, businessId, accountNumber, payerPhone);
    if (!client) {
      console.log("Jufinallies callback unmatched client", { transId, businessId, accountNumber, payerPhone });
    ...
    if (!loan) {
      ...
      const { error: chargeError } = await supabase
        ...
        .insert({
          ...
          description: `M-Pesa deposit for upfront loan charges. Account: ${accountNumber}. Payer: ${payerName}`,
      ...
      console.log("Jufinallies callback confirmed charge deposit", { transId, businessId, clientId: client.id, amount });
      return accepted();
    }

    if (!settings?.mpesa_auto_confirm) {
      console.log("Jufinallies callback queued for manual confirmation", { transId, businessId, loanId: loan.id });
      if (queue?.id) {
        await supabase
          .from("mpesa_callback_queue")
          .update({
    ...
    console.log("Jufinallies callback confirmed repayment", { transId, businessId, clientId: client.id, loanId: loan.id, repaymentId: repayment.id, appliedAmount, excessAmount, newBalance });
    return accepted();
  } catch (error) {
    console.error("Jufinallies C2B callback error:", error);
    if (trackedSupabase && trackedQueueId) {
      try {
        ...
        console.error("Jufinallies callback audit update failed", { trackedTransId, trackingError });
      }
    }
    return accepted();
  }
